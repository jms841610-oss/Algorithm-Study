...
int N; // 격자의 크기 (N x N)
int ans = 0; // 목적지에 도달하는 총 방법의 수

// 격자의 상태를 저장하는 2차원 배열 (0: 빈 칸, 1: 벽)
// 무한 루프 위험이 없으므로 방문 체크(is_visited) 배열은 삭제됨!
vector<vector<int>> area(16,vector<int>(16,0));

// DFS 탐색 함수: 파이프의 꼬리(tail)와 머리(head)의 좌표를 추적
void DFS(pair<int,int> tail, pair<int,int> head){
    // 현재 파이프 꼬리의 y(행), x(열) 좌표
    int tail_y = tail.first;
    int tail_x = tail.second;

    // 현재 파이프 머리의 y(행), x(열) 좌표
    int head_y = head.first;
    int head_x = head.second;

    // 1. 종료 조건 (Base Case)
    // 파이프의 꼬리나 머리 중 하나라도 목적지(N-1, N-1)에 도달하면 정답 카운트 1 증가 후 종료
    ...
    // tail_x != head_x 인 경우는 파이프가 '가로' 또는 '대각선' 상태임을 의미
    // (세로 상태일 때는 오른쪽으로 밀 수 없음)
    if(tail_x != head_x){
        // 머리가 오른쪽으로 이동해도 격자(N)를 벗어나지 않는지 확인
        if((head_y < N)&&(head_x+1 < N)){
            // 이동할 칸이 벽(1)이 아닌 빈 칸(0)인지 확인
            if(area[head_y][head_x+1] != 1){
                ...
                DFS(front_tail, front_head); // 다음 이동을 위해 DFS 재귀 호출
            } 
        }
    }
    ...
    // tail_y != head_y 인 경우는 파이프가 '세로' 또는 '대각선' 상태임을 의미
    // (가로 상태일 때는 아래쪽으로 밀 수 없음)
    if(tail_y != head_y){
        ...
        if((head_y+1 < N)&&(head_x < N)){
            ...
            if(area[head_y+1][head_x] != 1){
                ...
                DFS(down_tail, down_head); // 다음 이동을 위해 DFS 재귀 호출
            }
        }
    }
    ...
    // 대각선 이동은 파이프의 현재 상태(가로, 세로, 대각선)와 무관하게 항상 시도 가능
    // 머리가 대각선으로 이동했을 때 격자 범위를 벗어나지 않는지 확인
    if((head_y+1 < N)&&(head_x+1 < N)){
        // ⚠️ 중요: 대각선 이동 시에는 목적지뿐만 아니라, 그와 인접한 위/왼쪽 칸도 모두 빈 칸이어야 함!
        if((area[head_y+1][head_x] != 1)&&(area[head_y][head_x+1] != 1)&&(area[head_y+1][head_x+1] != 1)){
            
            // 꼬리는 이전 머리 위치로, 머리는 대각선(y+1, x+1)으로 한 칸 이동
            diag_head = {head_y+1, head_x+1};
            diag_tail = {head_y, head_x};

            DFS(diag_tail, diag_head); // 다음 이동을 위해 DFS 재귀 호출
        }
    }
}

int main(){
    // C++ 입출력 속도 향상을 위한 설정 (알고리즘 문제 풀이 시 권장)
